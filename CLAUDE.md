...
**Before touching any platform version**, read its CLAUDE.md first. It contains current dev/production status, local working paths, architecture, deploy commands, and key gotchas.

When implementing any new feature:

1. Extend `FEATURES.md` first — record the feature as planned.
2. Read the CLAUDE.md for each affected platform.
...
```
Read: store-locator-shopify/CLAUDE.md   ← for Shopify work
...
**Full guidance:** `store-locator-shopify/CLAUDE.md`
...
1. Read the CLAUDE.md for **each affected platform** before starting.
